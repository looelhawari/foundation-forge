...
export function ClientLogosShowcase() {
    const t = useTranslations('clientsShowcase');
    
    const logoItems = clientLogos.map((client, index) => (
        <div
            key={index}
        ...
        <section className="relative py-16 sm:py-24 md:py-32 bg-black overflow-hidden">
            ...
            <div className="container mx-auto px-4 sm:px-6 mb-12 md:mb-16">
                ...
                >
                    <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 px-4">
                        {t('trustedBy')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-500">{t('industryLeaders')}</span>
                    </h2>
                    <motion.div
                        initial={{ scaleX: 0 }}
            ...
            >
                <div className="flex justify-center gap-8 sm:gap-12 flex-wrap">
                    {[
                        { number: "45+", labelKey: "majorClients" },
                        { number: "90+", labelKey: "projectsDelivered" },
                        { number: "100%", labelKey: "satisfactionRate" }
                    ].map((stat) => (
                        <div
                            key={stat.labelKey}
                            className="text-center"
                        >
                            ...
                            <div className="text-sm sm:text-base text-gray-400">{t(`stats.${stat.labelKey}`)}</div>
                        </div>
                    ))}
                </div>
            </motion.div>
